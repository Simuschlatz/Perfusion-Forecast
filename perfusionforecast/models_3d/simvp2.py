...
class AttentionModule(nn.Module):
    ...
    def __init__(self, dim, kernel_size, dilation=3):
        ...
        self.conv1 = nn.Conv3d(dim, 2*dim, 1)
...
class GASubBlock(nn.Module):
    ...
    def forward(self, x):
        x = x + self.drop_path(
            self.layer_scale_1.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1) * self.attn(self.norm1(x)))
        x = x + self.drop_path(
            self.layer_scale_2.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1) * self.mlp(self.norm2(x)))
        return x
...
class SimVP_Model(nn.Module):
    ...
    def __init__(self, in_shape, hid_S=16, hid_T=256, N_S=4, N_T=4, model_type='gSTA',
        ...
        T, C, D, H, W = in_shape  # T is pre_seq_length
        D, H, W = int(D / 2**(N_S/2)), int(H / 2**(N_S/2)), int(W / 2**(N_S/2))  # downsample 1 / 2**(N_S/2)
        act_inplace = False
    ...
    def __init__(self,
        ...
        if upsampling is True:
            self.conv = nn.Sequential(*[
                nn.Conv3d(in_channels, out_channels*8, kernel_size=kernel_size,
