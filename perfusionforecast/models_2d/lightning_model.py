...
class Pl_Model(pl.LightningModule):
    ...
    @torch.no_grad()
    def overall_loss(self, loader, mode, use_wandb=False):
        for inputs, targets in loader:
    ...
    def predict_one_ct(self, path, input_frames, pred_frames, pred_n_frames_per_step):
        inputs = torch.from_numpy(np.load(path)).unsqueeze(1).unsqueeze(0).to(self.device)
        ...
        for d in range(inputs.shape[3]):
            ...
            for t in range(0, pred_frames, pred_n_frames_per_step):
                ...
                inputs_ = torch.cat([inputs_[:, pred_n_frames_per_step:, :, :], outputs_t], dim=1)
